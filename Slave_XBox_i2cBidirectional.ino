 ...
 */
 ...
 //
 //Note: This uses i2c communication 
 /////////////////////////////////////////////////////////////////////////////////////////////////////
...
void setup() {
  Serial.begin(115200); //MAY HAVE TO LOWER THIS TO 9600. But changing it may cause lag problems!
  ...
  Wire.begin(id);
  Wire.onRequest(requestEvent); //Event Writer/Transmitter
  Wire.onReceive(receiveEvent); //Event Reader/Receiver                        <------Added #####################
}
...
void loop() {
  ...
  if (Xbox.Xbox360Connected) {
    ...
    if(button1 == 1 || button2 == 2){ //This makes the vibrating stop and makes values nutral when button1 pressed.
      leftJoyY = 0;
      leftJoyX = 0;
      rightJoyY = 0;
      rightJoyX = 0;
...
void receiveEvent(int howMany){
  //if(button2 == 1){
    while(0 < Wire.available()){
     temp  = (Wire.read() << 8) | (Wire.read()); //0,1
    }
    Serial.print("  Temp: ");  Serial.println(temp);
  //}

  //Figure out a way to display this number or any other pieces of receiving data.
}
